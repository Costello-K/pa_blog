...
function PostCard({ post }) {
  const { id, author, images, tags, title, published_at, views, likes, dislikes, slug } = post;
  const { name, surname, nickname } = author;
  ...
  return (
    <PostContainer>
      ...
      <PostDetails>
        ...
        <div style={{width: "100%"}}>
          <PostAuthor>Created by: <LinkAuthor to={`/users/${author.id}/${nickname}`}>{userName}</LinkAuthor></PostAuthor>
